...
{
  extract($_GET);
  $gggg="select * from tbl_order inner join tbl_mastcart using(mastcart_id) where order_id='$order_id' and cart_status='Order Pick Up'";
  $fff=select($gggg);
              ...
              ?>
                        <?php 
                            if($row['cstatus']=="Assigned To Courier"){ ?>
                                <p><a href="?pickup=<?php echo $row['mastcart_id']; ?>&payment_id=<?php echo $row['Paym_Id']; ?>" class="btn btn-warning">Order Pick Up</a>
                                    
                                <a href="q.php?cust_id=<?php    echo $row['Cust_Id'] ?>&req=<?php   echo $row['Delivery_Id'] ?>" class="btn btn-warning">Product Details</a>    
                                </p>
                            <?php }
                            else if($row['cstatus']=="Order Pick Up"  and $row['reachable']!='Unreachable'){ ?>
                                <p><a href="?delivered=<?php echo $row['mastcart_id']; ?>&payment_id=<?php echo $row['Paym_Id']; ?>" class="btn btn-warning">Order Delivered</a></p>
                                <p><a class="btn btn-warning" href="?order_id=<?php echo $row['order_id'] ?>">Unreachable</a></p>
                        <?php    }
