  ...
  <body onload="printDiv()">
    ...
    <header class="clearfix">
      ...
      <div id="company" class="clearfix">
        ...
        <div> <?php  echo $row[0]['Cust_Fname'];?><?php  echo $row[0]['Cust_Lname'];?><br />               
     </div>
      </div>
      <div id="project">
        ...
        <div><span>PHONE NO:</span> +91 7510289032</div>
    ...
    $date=date('Y-m-d');
 $q = "SELECT * FROM `tbl_order` INNER JOIN `tbl_mastcart` USING (`mastcart_id`) INNER JOIN `tbl_childcart` USING (`mastcart_id`) INNER JOIN `tbl_item` USING (`Itm_Id`) INNER JOIN `tbl_customer` USING (`Cust_Id`)INNER JOIN tbl_purchild USING (`Itm_Id`) WHERE cust_Id='$cid' AND order_id='$id' GROUP BY`order_id`";
...
$res=select($q);
    ...
    foreach ($res as $row) {
     ...
     if(sizeof($res)>0)
     { ?>
       <tr><th colspan='4'>Total:</th><th> Rs. <?php echo $row['child_totamt'] ?> </th></tr>
     <?php } ?>
