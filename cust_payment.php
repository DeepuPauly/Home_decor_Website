...
if (isset($_POST['Payment'])) {
 ...
 echo $q="insert into tbl_order values(null,'$masterid',curdate(),'0')";
   $q1=insert($q);
    echo $qr="select * from tbl_card where card_no='$cardno'";
    $hh=select($qr);
      if(sizeof($hh)>0)
   {
$ooi=$hh[0]['card_id'];

   echo $q4="insert into tbl_payment values(null,'$q1','$ooi',curdate(),'$cid')";
     insert($q4);
     echo$q5="update tbl_mastcart set cart_status='Paid' where mastcart_id='$masterid'";
     update($q5);
       // return redirect('logindex.php');
     
   }
   else{
     echo $q2="insert into tbl_card values(null,'$cid','$cardno','$cardholder')";
      $q3=insert($q2);
      echo$q4="insert into tbl_payment values(null,'$q1','$q3',curdate(),'$cid')";
      insert($q4);
      echo$q5="update tbl_mastcart set cart_status='Paid',tot_amount='$newp' where mastcart_id='$masterid'";
      update($q5);
        // return redirect('logindex.php');
   ...
   foreach ($q7 as $res) 
    {
        // echo$qs="UPDATE `tbl_item` SET `stock`=`stock`-'".$res['quantity']."' WHERE Itm_Id='".$res['Itm_Id']."'";
        $qs="UPDATE `tbl_item` SET `stock`=`stock` - '".$res['quantity']."' WHERE Itm_Id='".$res['Itm_Id']."'";
      update($qs);
        $its=$res['Itm_Id'];

  $how="select * from tbl_item where Itm_Id='".$res['Itm_Id']."' and stock='0'";
     $r=select($how);
     if (sizeof($r)>0) 
     {
         $u="select * from tbl_purchild where Itm_Id='".$res['Itm_Id']."' and cpurch_status='available'";
        $i=select($u);
        if (sizeof($i)>0) 
        {
          $purcid=$i[0]['Pc_Id'];
          ...
          $selp=$i[0]['Selling_price'];
           echo$iupd="update tbl_item set Stock='$st',Price='$selp' where Itm_Id='".$res['Itm_Id']."'";
          update($iupd);
           $cupd="update tbl_purchild set cpurch_status='stock added' where Pc_Id='$purcid'";
          update($cupd);
        }else
        {
           $a=1;
...
{
    extract($_GET);
    $po="SELECT * FROM tbl_card INNER JOIN tbl_customer using (`Cust_Id`) where card_id='$id'";
    $gt=select($po);
}
            ...
            <form method="POST">
                ...
                <div class="row-1">
                  ...
                  <div class="row-1">
                    ...
                    <div class="row row-2">
                        <input type="text" name="cardno" maxlength="16" pattern="[0-9]{16}" placeholder="5134-5264-4" value="<?php if(isset($_GET['id'])){ 
                            echo $gt[0]['card_no'];
                        } ?>">
                    </div>
                </div>
                    ...
                    <div class="row row-2">
                        <input type="password" placeholder="***" pattern="[0-9]{3}" maxlength="3" minlength="3">
                    </div>
