  ...
  <form method="post">
    <table  class="table table-striped" style="width: 250px">
     ...
     <tr>
       ...
       <td align="center" colspan="3"><a href="purchase_report.php"><input type="button" class="btn btn-secondary" value="Print Report"></a></td>
      </tr>
...
<center>
        ...
        <table class="table table-striped" style="width:1450px">
         ...
         if (isset($_POST['sale'])) {
           ...
           if ($daily!="") {
             // "hi";
           $q="SELECT *,`tbl_purchild`.`Purch_quantity` AS cqnty FROM `tbl_purmaster` INNER JOIN `tbl_purchild` USING(`Pm_Id`) INNER JOIN `tbl_vendor` USING(`Ven_Id`) INNER JOIN `tbl_item` USING(`Itm_Id`) INNER JOIN `tbl_subcat` USING(`Subcat_Id`)where Pm_Date='$daily' and purch_status!='Pending'";
           }
            else if ($monthly!="") {
             ...
             // $q="SELECT *,`tbl_childpurch`.`purch_quantity` AS cqnty FROM `tbl_mastpurch` INNER JOIN `tbl_childpurch` USING(`mastpurch_id`) INNER JOIN `tbl_vendor` USING(`vendor_id`) INNER JOIN `tbl_item` USING(`item_id`) INNER JOIN `tbl_brand` USING(`brand_id`) where purch_date like '$monthly%' and purch_status!='Pending'";
               $q="SELECT *,`tbl_purchild`.`Purch_quantity` AS cqnty FROM `tbl_purmaster` INNER JOIN `tbl_purchild` USING(`Pm_Id`) INNER JOIN `tbl_vendor` USING(`Ven_Id`) INNER JOIN `tbl_item` USING(`Itm_Id`) INNER JOIN `tbl_subcat` USING(`Subcat_Id`) where Pm_Date like '$monthly%' and purch_status!='Pending'";
             ...
             // $q="SELECT *,`tbl_childpurch`.`purch_quantity` AS cqnty FROM `tbl_mastpurch` INNER JOIN `tbl_childpurch` USING(`mastpurch_id`) INNER JOIN `tbl_vendor` USING(`vendor_id`) INNER JOIN `tbl_item` USING(`item_id`) INNER JOIN `tbl_brand` USING(`brand_id`) where v_name like '$customer%' and purch_status!='Pending'";
                $q="SELECT *,`tbl_purchild`.`Purch_quantity` AS cqnty FROM `tbl_purmaster` INNER JOIN `tbl_purchild` USING(`Pm_Id`) INNER JOIN `tbl_vendor` USING(`Ven_Id`) INNER JOIN `tbl_item` USING(`Itm_Id`) INNER JOIN `tbl_subcat` USING(`Subcat_Id`)where Ven_Name like '$customer%' and purch_status!='Pending'";
